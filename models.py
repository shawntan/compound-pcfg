...
class CompPCFG(nn.Module):
    ...
    def sample(self, inp):
        ...
        t_emb = torch.cat([t_emb, z[:, None].expand(batch_size, self.t_states, self.z_dim)], 2)

        root_scores = F.log_softmax(self.root_mlp(root_emb), 1)
        rule_score = F.log_softmax(self.rule_mlp(nt_emb), 2)
        rule_scores = rule_score.view(batch_size, self.nt_states,
                                      self.all_states, self.all_states)
        unary_scores = F.log_softmax(self.vocab_mlp(t_emb), 2)
        # Order: nt_states, t_states
        start_state = root_scores[0].argmax(dim=-1)
        return self.recurse_sample(start_state, rule_score[0], unary_scores[0])
