...
class DropoutTokens(nn.Module):
    def __init__(self, dropout, unk_id):
        super(DropoutTokens, self).__init__()
        self.dropout = dropout
        self.unk_id = unk_id

    def forward(self, seq):
        if self.training and self.dropout > 0:
            #dropout_mask = torch.bernoulli(torch.full_like(seq, self.dropout,
            #                               dtype=torch.float32)).bool()
            dropout_mask = torch.empty_like(seq, dtype=torch.bool).bernoulli_(self.dropout)
            seq = seq.masked_fill(dropout_mask, self.unk_id)
        return seq
...
class OrderedMemoryRecurrent(nn.Module):
    ...
    def init_hidden(self, bsz):
        zeros = torch.zeros(bsz, self.nslot, self.slot_size,
                            device=self.input_projection[0].weight.device)
...
class OrderedMemory(nn.Module):
    def __init__(self, input_size, slot_size, nslot,
                 ntokens, padding_idx, word_dropout=0.2, dropout=0.2, dropoutm=0.1):

        super(OrderedMemory, self).__init__()
        self.padding_idx = padding_idx

        self.embedding = nn.Embedding(ntokens + 1, input_size,
                                      padding_idx=self.padding_idx)
        self.drop_tokens = DropoutTokens(dropout=0.1, unk_id=ntokens)
    ...
    def forward(self, input):
        ...
        X, _ = self.rnn_encode(X, mask)
        om_input, memories, probs = self.OM_forward(X, init_hidden, mask)
        self.attn = probs
...
class RNNContextEncoder(nn.Module):
    def __init__(self, input_size, slot_size, nslot,
        ...
        self.embedding = nn.Embedding(ntokens, 2 * input_size,
                                      padding_idx=self.padding_idx)
        self.rnn_encode = RNNEncoder(input_size, num_layers=3,
                                     dropout=dropoutm)
        self.expand = nn.Sequential(
            nn.Linear(input_size, 4 * input_size),
        )
        self.final_transform = nn.Sequential(
            nn.Linear(4 * input_size, slot_size),
            nn.LayerNorm(slot_size),
            nn.Tanh(),
            nn.Dropout(dropout)
        )
    ...
    def forward(self, input):
        ...
        summary = torch.max(self.expand(rnn_out), dim=0)[0]
        final_state = self.final_transform(summary)
...
class RNNEncoder(nn.Module):
    ...
    def forward(self, X, mask):
        ...
        # last = last[0][:, rev_sorted_idx, :], last[1][:, rev_sorted_idx, :]
