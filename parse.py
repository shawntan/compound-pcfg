...
import re
import pickle
from preprocess import get_tags_tokens_lowercase
...
def main(args):
    ...
    with torch.enable_grad():
        for tree in open(args.data_file, "r"):
            ...
            tree = tree.strip()
            _, sent_orig_, _ = get_tags_tokens_lowercase(tree)
            sent_orig = ' '.join(sent_orig_)
            ...
            # Insert replacement code for our model TODO
            # -------------------
            idxs, nodelist = model.infer(sents)
            root = idxpos2tree(sents[0].tolist(), idxs[0], nodelist,
                               return_wrapper=True,
                               vocab=None,
                               sub_words=sent)
            # print(root)
            span_b_set, _ = get_brackets(root.unwrap())
            argmax_spans = [{(x, y - 1) for x, y in span_b_set}]
            # -------------------
            ...
            for span in argmax_spans[0]:
                label_matrix[span[0]][span[1]] = 0
                binary_matrix[span[0]][span[1]] = 1
            binary_matrix[0][-1] = 1
            pred_tree = {}
